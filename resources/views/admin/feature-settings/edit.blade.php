...
@section('title', 'Edit Feature Setting')

@section('css')
<style>
    /* Icon Picker Modal Styles (same as create.blade.php) */
    .icon-picker-modal {
        max-height: 70vh;
    }
    .icon-picker-list {
    ...
    <main class="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="w-full max-w-xl bg-white shadow-xl rounded-lg overflow-hidden">
            <div class="px-8 py-6 border-b border-gray-200">
                <h2 class="text-2xl font-bold text-blue-800">Edit Feature Setting: <span class="text-orange-500">{{ $featureSetting->name }}</span></h2>
            </div>

            <form method="POST" action="{{ route('admin.feature-settings.update', $featureSetting) }}" class="p-8 space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Feature Name <span class="text-red-500">*</span></label>
                    <input type="text" id="name" name="name" value="{{ old('name', $featureSetting->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="description" name="description" value="{{ old('description', $featureSetting->description) }}" 
                           ...
                           placeholder="e.g., A Good High-Speed Wi-Fi">
                    @error('description')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-3">
                        <input type="text" id="icon" name="icon" value="{{ old('icon', $featureSetting->icon) }}" required
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        ...
                        <option value="1" {{ old('status', $featureSetting->status) == '1' ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ old('status', $featureSetting->status) == '0' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="pt-3">
                    <button type="submit"
                        ...
                        <i class="fas fa-save mr-2"></i> Update Feature Setting
                    </button>
                </div>
            </form>
        </div>
...
{{-- Icon Picker Modal (same as create.blade.php) --}}
...
@section('js')
<script>
// The JavaScript is identical to create.blade.php
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (iconInput) {
        updateIconPreview(iconInput.value); // Initial preview for edit page
        ...
        iconsToRender.forEach(icon => {
            ...
            iconDiv.addEventListener('click', function () {
                if (iconInput) {
                    ...
                    updateIconPreview(this.dataset.iconClass);
                }
