        ...
        <header class="bg-blue-700 text-white shadow-sm">
            <nav class="container mx-auto px-6 py-4 flex justify-between items-center">
                ...
                <a href="{{ route('admin.conference-rooms.index') }}"
                    ...
                    <i class="fas fa-arrow-left mr-1"></i> Back to Conference Room
                </a>
            </nav>
        </header>
        ...
        <main class="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
            <div class="w-full max-w-3xl bg-white shadow-xl rounded-lg overflow-hidden">
                <div class="px-8 py-6 border-b border-gray-200">
                    <h2 class="text-2xl font-bold text-blue-800">Edit Conference Room</h2>
                </div>

                <form method="POST" enctype="multipart/form-data"
                    action="{{ route('admin.conference-rooms.update', $conferenceRoom) }}" class="p-8 space-y-6">
                    @csrf
                    @method('PUT')

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700 mb-2">Conference Room
                                Name</label>
                            <input type="text" id="name" name="name"
                                value="{{ old('name', $conferenceRoom->name) }}" required
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" id="price" name="price"
                                    value="{{ old('price', $conferenceRoom->price) }}" required
                        ...
                        <div>
                            ...
                            <textarea id="intro" name="intro" rows="2"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-orange-500 focus:border-orange-500">{{ old('intro', $conferenceRoom->intro) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="2"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-orange-500 focus:border-orange-500">{{ old('description', $conferenceRoom->description) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex justify-center items-center px-6 pt-5 pb-6 border-2 @error('main_product_image') border-red-500 @else border-gray-300 @enderror border-dashed rounded-lg"
                                ...
                                {{-- Existing Image --}}
                                @if ($mainImage = $conferenceRoom->getFirstMedia('conference_primary_image'))
                                ...
                                <div class="text-center absolute inset-0 flex items-center justify-center bg-white bg-opacity-75"
                                    ...
                                    style="{{ $conferenceRoom->getFirstMedia('conference_primary_image') ? 'background: rgba(255,255,255,0.9)' : '' }}">
                        ...
                        <div>
                            <div class="mt-1 relative border-2 @error('additional_images.*') border-red-500 @else @error('additional_images') border-red-500 @else border-gray-300 @enderror @enderror border-dashed rounded-lg px-6 pt-5 pb-6"
                                ...
                                <div class="flex flex-wrap gap-4" id="preview-container">
                                    @foreach ($conferenceRoom->getMedia('conference_additional_images') as $media)
                                ...
                                <div class="text-center absolute inset-0 flex items-center justify-center bg-white bg-opacity-75"
                                    id="dropzone-content"
                                    style="{{ $conferenceRoom->getMedia('conference_additional_images')->count() ? 'background: rgba(255,255,255,0.9)' : '' }}">
                                    <div class="w-full p-4">
                                        <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none"
                                            viewBox="0 0 48 48">
                                            <path
                    ...
                    <div class="border-t pt-6">
                        ...
                        <div id="features-container" class="space-y-4">
                            @foreach (old('features', $conferenceRoom->features) as $index => $feature)
                    ...
                    <div class="mt-8">
                        <div class="px-4 py-4">
                            ...
                            <select name="is_active" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                                <option value="1" {{ $conferenceRoom->is_active ? 'selected' : '' }}>Yes</option>
                                <option value="0" {{ !$conferenceRoom->is_active ? 'selected' : '' }}>No</option>
                            </select>
                        </div>
                        <button type="submit"
...
@section('js')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let featureIndex = {{ count(old('features', $conferenceRoom->features)) }};
