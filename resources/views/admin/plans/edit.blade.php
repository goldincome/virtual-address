...
@section('title', 'Edit Virtual Address Plan')
@section('content')
        ...
        <main class="flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
            <div class="w-full max-w-3xl bg-white shadow-xl rounded-lg overflow-hidden">
                <div class="px-8 py-6 border-b border-gray-200">
                    <h2 class="text-2xl font-bold text-blue-800">Edit Plan</h2>
                </div>

                <form method="POST" enctype="multipart/form-data" action="{{ route('admin.plans.update', $plan) }}"
                    class="p-8 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="2"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-orange-500 focus:border-orange-500">{{ old('description', $plan->description) }}</textarea>
                        </div>

                        <div>
                            <label for="price"
                                class="block text-sm font-medium text-gray-700 mb-2">Monthly Price({{ config('cashier.currency') }})</label>
                            <div class="relative">
                        ...
                        <div>
                            <label for="yearly_monthly_price"
                                class="block text-sm font-medium text-gray-700 mb-2">Yearly Monthly Price({{ config('cashier.currency') }})</label>
                            <div class="relative">
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex justify-center items-center px-6 pt-5 pb-6 border-2 @error('main_product_image') border-red-500 @else border-gray-300 @enderror border-dashed rounded-lg"
                                ...
                                {{-- Existing Image --}}
                                @if ($mainImage = $plan->getFirstMedia('plan_primary_image'))
                                ...
                                <div class="text-center absolute inset-0 flex items-center justify-center bg-white bg-opacity-75"
                                    ...
                                    style="{{ $plan->getFirstMedia('plan_primary_image') ? 'background: rgba(255,255,255,0.9)' : '' }}">
                        ...
                        <div>
                            <div class="mt-1 relative border-2 @error('additional_images.*') border-red-500 @else @error('additional_images') border-red-500 @else border-gray-300 @enderror @enderror border-dashed rounded-lg px-6 pt-5 pb-6"
                                ...
                                <div class="flex flex-wrap gap-4" id="preview-container">
                                    @foreach ($plan->getMedia('plan_additional_images') as $media)
                                ...
                                <div class="text-center absolute inset-0 flex items-center justify-center bg-white bg-opacity-75"
                                    id="dropzone-content"
                                    style="{{ $plan->getMedia('plan_additional_images')->count() ? 'background: rgba(255,255,255,0.9)' : '' }}">
                                    <div class="w-full p-4">
                                        <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none"
                                            viewBox="0 0 48 48">
                                            <path
                        ...
                        <div>
                            <label for="level"
                                class="block text-sm font-medium text-gray-700 mb-2">Hierachy Level</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-500">

                                </span>
                                <input type="number" step="1" id="level" name="level" required value="{{ old('level', $plan->level) }}"
                                    class="w-full px-4 py-2 border @error('level') border-red-500 @else border-gray-300 @enderror rounded-lg shadow-sm focus:ring-orange-500 focus:border-orange-500 transition duration-150 ease-in-out">
                                @error('level')
                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div>
                            <label for="is_active"
                                class="block text-sm font-medium text-gray-700 mb-2">Enabled?</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-500">

                                </span>
                                <select id="is_active" name="is_active" required
                                        class="w-full px-4 py-2 border @error('is_active') border-red-500 @else border-gray-300 @enderror rounded-lg shadow-sm focus:ring-orange-500 focus:border-orange-500 transition duration-150 ease-in-out">
                                    <option value="1" {{ old('is_active', $plan->is_active) == '1' ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ old('is_active', $plan->is_active) == '0' ? 'selected' : '' }}>No</option>
                                </select>
                                @error('is_active')
                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                @enderror
...
@section('js')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let featureIndex = {{ count(old('features', $plan->features)) }};
            ...
            const yearlyPriceInput = document.getElementById('yearly_monthly_price');

            // Listen for the 'input' event on the monthly price field
            priceInput.addEventListener('input', function() {
                // Get the value from the price input and convert it to a number
                const monthlyPrice = parseFloat(priceInput.value);
                ...
                if (!isNaN(monthlyPrice) && monthlyPrice >= 0) {
                    ...
                    // Update the yearly price input, formatted to 2 decimal places
                    yearlyPriceInput.value = yearlyPrice.toFixed(2);
                } else {
                    // If the input is empty or not a number, clear the yearly price
