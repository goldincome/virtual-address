...
@section('content')
    <div class="container mx-auto px-6 py-16 md:py-10 max-w-7xl">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-2 space-y-10">
                ...
                <section id="room-images">
                    <h2 class="text-3xl font-bold text-blue-800 mb-6">{{ $conferenceRoom->name }} Gallery</h2>
                    <div class="mb-6">
                        <img id="main-room-image" src="{{ $conferenceRoom->conference_primary_image ?: 'https://placehold.co/1080x720/1d4ed8/ffffff?text=Room+Image' }}"
                            alt="{{ $conferenceRoom->name }} - Main View"
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 thumbnail-container">
                        <img src="{{ $conferenceRoom->conference_primary_image ?: 'https://placehold.co/150x100/cccccc/ffffff?text=Thumb+1' }}"
                            alt="{{ $conferenceRoom->name }} Thumbnail"
                            data-large="{{ $conferenceRoom->conference_primary_image ?: 'https://placehold.co/1080x720/cccccc/ffffff?text=Image+1+Not+Found' }}"
                        ...
                        @foreach ($conferenceRoom->getMedia($conferenceRoom::CONFERENCE_ADDITIONAL_IMAGES) as $media)
                            <img src="{{ $media->getUrl() }}" alt="{{ $conferenceRoom->name }} Thumbnail {{ $loop->iteration + 1 }}"
                ...
                <section id="room-description">
                    ...
                    <div class="prose max-w-none text-gray-700 leading-relaxed space-y-4">
                        {!! $conferenceRoom->description ?: '<p>Detailed description not available. Please contact us for more information.</p>' !!}
                    </div>
                </section>
                ...
                <section id="room-features">
                    ...
                    @if($conferenceRoom->features && (is_array($conferenceRoom->features) ? count($conferenceRoom->features) : (is_object($conferenceRoom->features) && method_exists($conferenceRoom->features, 'count') ? $conferenceRoom->features->count() : 0)) > 0)
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                            {{-- Adjust based on how $conferenceRoom->features is structured --}}
                            {{-- If it's a collection of objects with a 'value' property, as in the old file: --}}
                            @foreach($conferenceRoom->features as $feature)
                                <div class="flex items-center text-gray-700">
                                    <i class="{{ $feature->featureSetting->icon }} text-orange-500 mr-3 w-6 text-center"></i>
                                    <span>{{ $feature->featureSetting->name }}</span> {{-- Adapt if structure is different --}}
                                </div>
                            @endforeach
                        </div>
                    @else
                         <div class="grid grid-cols-2 sm:grid-cols-3 gap-6">
                            <div class="flex items-center text-gray-700"><i class="fas fa-users text-2xl text-orange-500 mr-3 w-8 text-center"></i><span>Capacity: Varies</span></div>
                            <div class="flex items-center text-gray-700"><i class="fas fa-wifi text-2xl text-orange-500 mr-3 w-8 text-center"></i><span>High-Speed Wi-Fi</span></div>
                            <div class="flex items-center text-gray-700"><i class="fas fa-video text-2xl text-orange-500 mr-3 w-8 text-center"></i><span>AV Equipment</span></div>
                        </div>
            ...
            <div class="lg:col-span-1 space-y-10">
                <section id="booking" class="bg-white p-6 rounded-lg shadow-lg border border-gray-200 sticky top-24">
                    <h3 class="text-2xl font-semibold text-blue-800 mb-4 text-center">Book {{ $conferenceRoom->name }}</h3>
                    @include('admin.common.error-and-message')
                    <div class="text-center mb-5">
                        <span class="text-3xl font-bold text-blue-700">{{ currencyFormatter($conferenceRoom->price) }}</span>
                        <span class="text-gray-600">/ hour</span>
                    </div>

                    <form action="{{ route('conference-rooms.store', ['product_id' => $conferenceRoom->id] ) }}" method="POST" class="space-y-4">
                        @csrf
                ...
                <section id="why-choose">
                    <h3 class="text-2xl font-semibold text-blue-800 mb-4">Why Choose {{ $conferenceRoom->name }}?</h3>
        ...
        {{-- CTA and Other Rooms Sections remain the same --}}
         <section id="cta-booking" class="mt-16 md:mt-24 py-12 bg-blue-600 text-white rounded-lg shadow-xl text-center">
            <div class="container mx-auto px-6">
                <h3 class="text-3xl font-semibold mb-4">Ready to Book {{ $conferenceRoom->name }}?</h3>
        ...
        <section id="other-rooms" class="mt-16 md:mt-24 text-center">
            ...
            <div class="flex justify-center space-x-6">
                ...
                <a href="{{ url('/conference-rooms') }}" class="text-blue-600 hover:text-orange-500 hover:underline transition duration-300"><i class="fas fa-chalkboard-teacher mr-1"></i> View All Conference Rooms</a>
            </div>
        </section>
    </div>
    ...
    <div id="imageModal" class="modal">
        ...
        <div class="modal-content-wrapper bg-white p-4 rounded-lg">
            <img class="modal-content" id="modalImage" src="" alt="Conference Room View">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function updateBookingTimeSlots(selectedDate) {
                ...
                fetch(`/schedule/slots?product_id={{ $conferenceRoom->id }}&booking_date=${selectedDate}`)
                    .then(response => {
                        if (!response.ok) {
